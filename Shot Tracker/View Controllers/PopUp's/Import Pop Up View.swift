...
class Import_Pop_Up_View: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    // convert csv files to string then convert [[string]] to overall stats table in realm
    ...
    // convert csv files to string then convert [[string]] to faceoff table in realm
    func csvStringToRealmFaceoffStatsTable() -> Bool{
        // get document path based on search for spefic file
        let documentsUrl = documentURLProducer()
        let eightDocumentPath = documentsUrl.appendingPathComponent(fileCollection()[fileCollection().firstIndex(of: "Realm_Faceoff_Stats_Table.csv")!])
        
        var eightFileContentsParsed: [[String]] = [[String]]()
        // get contents of specfic csv file and place into array above
        do {
            eightFileContentsParsed =  (try String(contentsOf: eightDocumentPath, encoding: .utf8)).components(separatedBy: "\n").map{ $0.components(separatedBy: ",") }
        } catch {
            print("Error Finding Containts of File")
        }
        
        if (eightFileContentsParsed[0].count == 6){
            try? realm.write ({
                //delete contents of table in realm DB
                realm.delete(realm.objects(faceOffInfoTable.self))
                
                for i in 1..<eightFileContentsParsed.count - 1{
                    if(eightFileContentsParsed[i].contains{$0 != ""}){
                        var primaryID: Int!
                        if (self.realm.objects(faceOffInfoTable.self).max(ofProperty: "faceoffID") as Int? != nil){
                            primaryID = (self.realm.objects(faceOffInfoTable.self).max(ofProperty: "faceoffID") as Int? ?? 0) + 1;
                        }else{
                            primaryID = (self.realm.objects(faceOffInfoTable.self).max(ofProperty: "faceoffID") as Int? ?? 0);
                        }
                        self.realm.create(faceOffInfoTable.self, value: ["faceoffID": primaryID])
                        let primaryOverallID = self.realm.object(ofType: faceOffInfoTable.self, forPrimaryKey: primaryID)
                        var count = 0
                        primaryOverallID?.gameID = Int(eightFileContentsParsed[i][count])!; count += 1
                        primaryOverallID?.winingPlayerID = Int(eightFileContentsParsed[i][count])!; count += 1
                        primaryOverallID?.losingPlayerID = Int(eightFileContentsParsed[i][count])!; count += 1
                        primaryOverallID?.periodNum = Int(eightFileContentsParsed[i][count])!; count += 1
                        primaryOverallID?.faceoffLocationCode = Int(eightFileContentsParsed[i][count])!; count += 1
                        if (Bool(eightFileContentsParsed[i][count]) != nil && Bool(eightFileContentsParsed[i][count])!){
                            primaryOverallID?.activeState = (Bool(eightFileContentsParsed[i][count]))!; count += 1
                        }else{
                            let set = CharacterSet(charactersIn: "truefalse")
                            primaryOverallID?.activeState = Bool((eightFileContentsParsed[i][count]).lowercased().components(separatedBy: set.inverted).joined())!; count += 1
                        }
                    }else{
                        acceptedIncorrectDataFormat(fileType: "Faceoff Stats Table")
                        break
                    }
                    
                }
            })
            print("Faceoff Stats Table Success")
            return(true)
        }else{
            incorrectDataFormat(fileType: "Faceoff Stats Table")
            return(false)
        }
    }
