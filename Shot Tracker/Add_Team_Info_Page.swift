...
class Add_Team_Info_Page: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate {
...
// on add player button click
    @IBAction func savePlayer(_ sender: Any) {
        ...
        let line = selectLine
        let primaryPlayerID = (realm.objects(playerInfoTable.self).max(ofProperty: "playerID")as Int? ?? 0) + 1
        ...
        if (number != nil && player != nil){
            ...
            try! realm.write{
                // write info to relam and reset all fields
